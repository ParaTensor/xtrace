 ...
 * - Explicit `VITE_XTRACE_BASE_URL` wins (use `""` for same-origin / Vite proxy).
 * - In Vite `dev`, default to same-origin so remote port-forward of only :5173 works.
 * - Otherwise default to local API.
 */
export const getBaseUrl = () => {
  const configured = import.meta.env.VITE_XTRACE_BASE_URL;
